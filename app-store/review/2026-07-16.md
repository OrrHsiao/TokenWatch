...
- Each standard NSOpenPanel is shown only after the user clicks that provider’s visible authorization action (Authorize for an unselected folder, or Choose Again when a saved bookmark must be renewed).
...
3. For any provider marked Unauthorized, click Authorize and confirm that a standard folder picker appears with provider-specific instructions. The app does not set the initial location; macOS controls the location initially displayed by the panel.

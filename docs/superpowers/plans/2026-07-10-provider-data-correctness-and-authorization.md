...
**Architecture:** 两份计划只在一个明确断点交错：先完成计价计划 Task 1–2，再执行本计划 Task 1，使 `TokenUsage.cacheCreation` 在任何 `nil` call site 出现前成为 optional；随后暂停本计划，完成计价计划 Task 3–6 及阶段验证，最后回到本计划 Task 2–7。实现以小型纯类型隔离墙上小时描述和 Claude 去重，以共享 `JSONLFileReading`/`JSONLByteStream` 隔离文件元数据与字节读取，并由聚焦的泛型 `JSONLLastGoodCacheCoordinator` 统一 Claude/Codex 的 snapshot、cache-hit、last-good、scope 与 prune 协调；provider-specific candidate、解析 reducer 与最终去重继续保留在各自 parser。bookmark 创建与存储则通过两个窄依赖注入到现有 manager。

**Tech Stack:** Swift 6、Foundation、AppKit、OSLog、SQLite3、Swift Testing、Xcode 26.5/macOS 15。

## Global Constraints

- 跨计划唯一执行序列为：计价计划 Task 1 → 计价计划 Task 2 → **本计划 Task 1** → 计价计划 Task 3 → Task 4 → Task 5 → Task 6 及其阶段验证 → **本计划 Task 2 → Task 3 → Task 4 → Task 5 → Task 6 → Task 7**。
- 本计划 Task 1 只依赖计价计划 Task 1–2，不依赖尚未创建的 `CCUsagePricingParityTests.swift` 或金额 fixture；Task 1 GREEN 后必须回到计价计划 Task 3，不能继续执行本计划 Task 2。
- 本计划 Task 2 的前置门禁是计价计划 Task 3–6 与固定 `ccusage v20.0.16 / e32cc482 --offline` 金额 fixture 全部通过；之后本计划不得回退其 Auto、模型查找、tier、fast 或 fallback 语义。
...
- Claude/Codex 的 cache-hit、last-good、scope 匹配、scanner prune 与原子替换必须由同一个 `JSONLLastGoodCacheCoordinator<Candidate, Scope>` 实现；两个 parser 不得各自保留近乎逐字重复的协调循环，但各自的 raw candidate、line parser、checkpoint reducer 和最终去重仍保持 provider-specific。
...
- `TokenWatch/Providers/JSONLLastGoodCacheCoordinator.swift`：以 `Candidate`/`Scope` 泛型统一 listed-file 遍历、同 descriptor metadata cache hit、scope-sensitive last-good、成功后原子替换和 scanner prune；不解析 provider 行，也不执行 provider 全局去重。
...
- `TokenWatch/Providers/Claude/ClaudeJSONLParser.swift:14-242`：执行 daily 过滤、传播 sidechain、调用单遍 deduplicator，并把 provider-specific 解析 closure 交给共享 reader/coordinator；不再自有 cache 协调循环。
- `TokenWatch/Providers/Codex/CodexRolloutParser.swift:14-250`：保留 last-first reducer、clamp cached、执行 ccusage-compatible 跨文件去重与 replay 跳过，并把 `CodexUsageCandidate` 解析 closure 与 `CodexPricingSpeed` scope 交给共享 reader/coordinator；不再自有 cache 协调循环。
...
**Interfaces:**
- Consumes: 已 GREEN 的计价计划 Task 1–2 与当前既有 `PricingEngine.calculateCost(usage:pricing:)`；不消费尚未由计价 Task 6 创建的 `CCUsagePricingParityTests.swift` 或金额 fixture。
- Produces: `TokenUsage.cacheCreation: CacheCreation?`；`ServerToolUse.init(webSearchRequests:webFetchRequests:)`；`CacheCreation.init(ephemeral1hInputTokens:ephemeral5mInputTokens:)`；presence-aware `cacheCreate5mTokens`、`cacheCreate1hTokens`、`totalCacheCreationTokens`。这些接口是计价计划 Task 3–6 所有 `cacheCreation: nil` call site 的强制前置。
...
`CCUsagePricingParityTests.swift` 此时尚不存在，不在本 Task 创建或修改。稍后计价计划 Task 6 创建其 `entry(from:)` 时，必须直接使用本 Task 已落地的 optional initializer：Claude fixture 构造非 nil breakdown，Codex/OpenCode 传 `nil`；不得在这里预写 future-file 变更。
...
Expected: PASS；缺失 breakdown 走 flat，存在空对象不回退，两个 provider 的扁平 usage 为 nil，既有 PricingEngine 回归保持通过。固定金额 fixture 尚不存在，留到计价计划 Task 6 创建后再验证，不能在这里使用一个会静默匹配零测试的 selector。
...
### 跨计划交接：Task 1 完成后返回计价计划

本计划在这里强制暂停。先回到 `docs/superpowers/plans/2026-07-10-ccusage-pricing-parity.md`，从其 Task 3 顺序执行到 Task 6，并完成该文件的阶段验证。只有固定金额 fixture 与完整计价定向 suite GREEN 后，才从下方 Task 2 继续本计划。不要从 Provider Task 1 直接执行 Provider Task 2，也不要在 Provider Task 1 前执行任何含 `cacheCreation: nil` 的计价步骤。
...
Expected: RED 来自 cached 未 clamp、replay 未分类、跨 session 未去重，或 `CodexRecordTests` 新增的 timestamp/alias/lossy number/total normalization/model 边界；Step 1 的全部 parser 与 record fixture 都必须在生产实现前实际执行。`repeatedTotalStillEmitsNonzeroLastUsage` 应在移除旧计划的自定义抑制后锁定为 PASS。
...
### Task 6: 落地共享 JSONL reader 与泛型 last-good 协调器

**Files:**
- Create: `TokenWatch/Providers/JSONLFileReader.swift`
- Create: `TokenWatch/Providers/JSONLLastGoodCacheCoordinator.swift`
...
- Create: `TokenWatchTests/Providers/JSONLLastGoodCacheCoordinatorTests.swift`
...
- Produces: `JSONLFileIdentity`、`JSONLFileMetadata`、`JSONLByteStream`、`JSONLFileReading`、`SystemJSONLFileReader`；`JSONLLastGoodCacheCoordinator<Candidate, Scope>.loadListedFiles(...)`；Claude/Codex `init(fileReader:)`；发现文件失败时返回 scope-compatible per-file last-good candidates。`Candidate` 仍分别为 `[ParsedUsageEntry]` 元素和 `CodexUsageCandidate`，`Scope` 分别为 `JSONLUnscopedCacheScope` 和 `CodexPricingSpeed`；共享 helper 不解析行、不认识 provider、不执行 Claude/Codex 全局去重。下一阶段增量解析必须原样复用 reader 签名，并在扩展 cache state 时保留这一协调边界。

- [ ] **Step 1: 先写生产 reader、共享协调器与可控失败 seam 的测试**
...
创建 `TokenWatchTests/Providers/JSONLLastGoodCacheCoordinatorTests.swift`，直接锁定 shared helper 的职责边界；这个测试必须先于 helper production code：
...
xcodebuild -project TokenWatch.xcodeproj -scheme TokenWatch -destination 'platform=macOS' \
  ...
  -only-testing:TokenWatchTests/JSONLLastGoodCacheCoordinatorTests \
...
Expected: FAIL；共享 reader、`JSONLLastGoodCacheCoordinator` 和 injectable initializer 尚不存在。当前已发现文件读取失败会整文件丢失，且 Claude/Codex 各自的 cache loop 没有一个可共同验证的 scope/prune/atomic-replacement 契约。
...
- [ ] **Step 5: 实现最小共享泛型 cache 协调器**

创建 `TokenWatch/Providers/JSONLLastGoodCacheCoordinator.swift`。这个类型只拥有 cache 生命周期；它不知道 JSONL 行形状、Claude/Codex candidate 字段或最终去重算法：
...
/// `Candidate` 和 `Scope` 由 provider 指定；行解析和全局去重通过 closure 留在 provider。
...
>: @unchecked Sendable {
    ...
    /// 读取 scanner 本轮列出的文件，并返回仍未做 provider 全局去重的 candidates。
...
`loadListedFiles` 是唯一 cache 协调循环。helper 必须在 parse 完整返回后才 `store`，catch 绝不写 cache；即使 last-good candidates 是空数组，也要把“存在 last-good”和“没有 last-good”区分开。metadata identity 为 nil 时不得命中 unchanged cache，但成功 parse 仍可成为后续读取失败时的 last-good。

- [ ] **Step 6: 让两个 parser 只提供 candidate parser、scope 与最终去重**

删除两个 parser 自有的 `FileSignature`、`CachedFile`、`cachedFiles`、cache lock/hit counter、`cachedCandidates`、`lastGoodCandidates`、`parseCached...`、`pruneCache` 和 per-file `do/catch` 循环。两者保留 `fileReader` 只服务公开单文件 API，并把同一个 reader 注入 coordinator。

在 `ClaudeJSONLParser` 中使用完整依赖与 debug 转发：
...
在 `CodexRolloutParser` 中使用 provider-specific raw candidate 与 speed scope：
...
`parseJSONLStream` 与 `parseCandidates` 仍各自拥有 provider-specific `processLine`/reducer。它们只把输入从 `FileHandle` 改为 `any JSONLByteStream`，并使用同一 descriptor stream：
...
Claude 的 `processLine/parseCandidate` 使用绝对 `sourceOffset` 生成 missing-ID identity；Codex 可把 offset 用作本地 record identity 的稳定辅助值，但 session token_count timestamp 缺失时仍按 Task 4 跳过。Claude 最终调用 `ClaudeUsageDeduplicator`，Codex 最终执行 `CodexEventDedupKey` first-wins；两种语义不得移入 generic coordinator。`CodexPricingSpeed` 作为 scope 保证 speed 切换后的读取失败不会复用旧 tier last-good。
...
Expected: PASS；shared coordinator 证明 unchanged 文件读取 0 字节、stat/open/seek/read 任一失败都走同一个 last-good 分支、scope 不匹配不复用、首次失败为空、成功 parse 才替换、scanner 删除立即 prune；Claude/Codex parser suite 继续证明各自 candidate 与最终去重语义未被泛型化。
...
git add TokenWatch/Providers/JSONLFileReader.swift \
  ...
  TokenWatchTests/Providers/Claude/ClaudeJSONLParserTests.swift \
  TokenWatchTests/Providers/Codex/CodexRolloutParserTests.swift
git commit -m "fix(parser): 共享 JSONL last-good 缓存协调"
```
...
4. `JSONLLastGoodCacheCoordinator<Candidate, Scope>` 是唯一 cache 协调循环；Claude/Codex parser 只保留 provider-specific candidate 解析与最终去重。
5. Codex previousTotals 为 optional，并由增量 checkpoint 在 committed offset 保存。
6. last-good 只覆盖 scanner 已发现但随后读取失败的文件；空 scanner 列表立即 prune，Codex 不跨 pricing speed 复用。
7. bookmark creator/store 任一失败都返回 nil，ViewModel 不进入成功刷新分支。
```

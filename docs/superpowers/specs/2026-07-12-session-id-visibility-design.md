...
布局断言：定位缩略 ID 对应的内部 `NSTextField`、复制图标及其间距约束，用标题 fitting
宽度、实际间距和图标宽度计算所需内容宽度，并验证按钮铺满固定 150 pt 的 ID 单元格。
旧约束下按钮只有 134 pt，小于 119 + 6 + 13 = 138 pt；尾部等式约束生效后按钮扩展为
150 pt，足以完整容纳标题和图标。
...
复制完整 ID”的现有语义一致。测试将继续覆盖复制结果与标题、图标的布局容量，避免交互
与裁剪回归。

...
- LiteLLM `provider_specific_entry.fast` 优先，builtin fast override 只填缺失值。
...
1. 用原始 input，也就是 `pure input + cached input`，判断是否大于 272K。
...
`ParsedUsageEntry` 增加记录级 `isSidechain`（其他 provider 默认为 false），Claude 从 `ClaudeRecord.isSidechain` 传播；per-file cache 保存尚未全局去重的 entry candidates，不能把文件级 `isSubagent` 当作 sidechain。`UsageEntriesFingerprint` 与测试 deep snapshot 同步包含该字段。

第一层继续按 `messageId + requestId` 精确找 duplicate。第二层只对同 `messageId` 且任一 candidate `isSidechain == true` 的 replay 建立额外 duplicate 关系。无论由哪一层找到 duplicate，都使用同一 replacement 顺序：
...
已有 `previousTotals` 且新 `total_token_usage` 完全相同时，不再次发出相同 `last_token_usage`。首条事件、累计增长和 total 缺失的既有 fallback 行为保持不变。该规则是额外稳健性保护，不属于 ccusage 计价契约。
...
首次从未成功解析的坏文件继续跳过。scanner 已返回某个 `fileInfo`，但随后 stat/seek/read 失败且该 cache key 已有上次成功结果时，复用 last-good candidates 并记录 warning，避免部分列表覆盖完整统计。scanner 未返回的文件仍按真实删除处理并 prune；本轮不引入 tombstone 或 grace period。
...
追加解析从该状态恢复，保证后续 delta 与模型归属和全量解析一致。
...
移除 `DashboardNavigationButton`、`DashboardRangeButton`、`DashboardSessionButton` 永久拒绝 first responder 的实现，恢复键盘聚焦和 focus ring。窗口启动时通过初始 responder 配置避免自动选中操作按钮，而不是禁用按钮的键盘能力。
...
1. **计价 parity**：Auto、模型查找、cache 默认、Claude/Codex 两类 tier、fast、fallback 与固定金额 fixture。
2. **数据正确性与授权**：DST、宽容解码、sidechain、Codex 防重、OpenCode、last-good cache、bookmark。
...
5. 通过可注入 reader 或 debug read metrics 验证：unchanged 文件读取 0 字节；append 的首次读取 offset 等于 committed offset，读取量仅为 tail + suffix；测试不能仅比较最终结果。

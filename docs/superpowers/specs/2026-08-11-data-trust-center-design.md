...
3. 任一已接入 Provider 无当前或旧结果且无法读取：`limited`。
4. 任一 Provider 使用 last-good/旧汇总、存在源被跳过、异常 usage 候选、扫描/费用诊断未知、费用未覆盖、本地汇总快照写入失败或数据过期：`attention`。
...
| Provider 未接入 | 未接入 | 不参与总体结论 | 前往设置 |

### 新鲜度规则

- 自动刷新关闭时，只展示时间事实，不因时间流逝降低状态。
- 自动刷新开启时，App 处于 active 且 `now - lastSuccessfulScanAt > max(2 × refreshInterval, 10 分钟)` 才标记过期。
- App 从睡眠/后台恢复后先安排刷新，在首次刷新完成前不立即制造过期警告。
- 最近一次尝试失败时，无论时间阈值如何都进入 attention/limited。
...
struct PricingMatch: Sendable, Equatable {
    ...
    let catalog: PricingCatalogSource
...
1. `upstreamCost != nil` 立即返回 `.upstreamReported`，包括显式 `0`。
2. 本地价格命中返回 `.localEstimate`，金额继续遵守当前 provider/ccusage 语义。
3. 没有 upstream 且所有候选都未命中价格时返回 `.unavailable`，金额仍为 `0`，但 UI 不再解释为免费。
4. OpenCode 继续保留“候选中正金额优先”的金额选择契约；OpenCode 源数据的 `cost <= 0` 仍按现有 adapter 规则转为 `nil`。如果所有本地命中候选金额都是 `0`，仍记录首个 `pricing != nil` 的匹配作为已覆盖，避免用金额正负判断价格存在性。
5. `PricingTable` 的内部 value 从裸 `ModelPricing` 升级为带 catalog provenance 的条目，并提供 `lookup(_:) -> PricingLookupResult?`，返回实际 matched key 与 exact/normalized-boundary 类型；alias 和 OpenCode provider candidate 属于 resolver 路径，不能伪装成表内 match。UI P0 默认只展示“本地内置价格”，详情模型保留完整审计信息。
...
- 只有没有 last-good、无源跳过、无异常 usage 拒绝时才更新 `lastCompleteFreshScanAt`。
...
- `sourceRevision`
- `pipelineRevision`
- 最近一次成功且 parse 完整的 `ProviderScanDiagnostics`
...
v2 将现有明文 `dataRootPath` 改为 `dataRootFingerprint`：以 Provider ID 和标准化数据根生成 SHA-256，只用于判断当前授权根是否相同，避免在快照头部重复保存完整路径。它只是数据最小化措施，不是加密或安全边界；`AggregatedStats` 的项目 breakdown 仍可能包含 cwd，因此隐私披露与 P0-5 清理范围不能缩小。
...
- bundled pricing artifact hash。
- fast multiplier、alias 和 auto-review 映射版本。

不能直接使用 App build/version，否则纯 UI 更新也会无意义地重算；也不能把扫描时间或诊断计数加入 revision。

实现上由生产代码暴露稳定的 parser/dedup/aggregation/resolver 版本常量和内置 catalog hash，再按固定字段顺序生成 SHA-256。catalog hash 不应在每次刷新时重新扫描资源；构建期/静态常量负责提供值，测试再用实际资源内容校验常量没有漂移。
...
- 只有 Bookmark 成功恢复 security-scoped URL、`validateDataRoot` 通过，并且由该 URL 计算出的 `dataRootFingerprint`、时区和 pipeline 都与快照兼容时，本轮扫描抛错才允许展示磁盘聚合快照。
- Bookmark 无法恢复、权限失效或目录结构错误时绝不加载快照，因为无法证明它属于用户当前授权的根；该 Provider 清空展示数据并进入受限状态。
- 用户选择新目录后先比较 fingerprint；根发生变化时，在新扫描提交前原子清空该 Provider 的 retained entries/stats/trust、entry fingerprint、source revision 和旧快照，绝不让旧根数据显示在新根名下。
...
- 实施前先冻结新增 key 清单；新增 enum case、65 份资源、硬编码 key-count/英文复用 allowlist 更新和首次 UI 引用放在同一个可验证提交，不能留下中间必失败的本地化测试。
...
详细设计确认后，实施计划按以下可回滚提交拆分：

1. `fix(dashboard)`：修正范围模型排行、记录口径与费用来源文案。
2. `feat(diagnostics)`：新增诊断领域模型、状态 builder 与真值表测试。
3. `feat(provider)`：共享 JSONL coordinator 与三 Provider 上送结构化诊断。
4. `feat(pricing)`：结构化费用来源、catalog provenance 与覆盖率聚合。
5. `feat(snapshot)`：ProviderStatsSnapshot v2、pipeline revision、时间与旧数据语义。
6. `fix(privacy)`：原始 tail/anchor 不落盘及缓存迁移回归。
7. `feat(data-health-ui)`：独立页面、导航、深链、状态、动作、冻结后的全部语言 key/资源与无障碍语义。
8. `docs(privacy)`：同步 App、仓库和站点隐私文案。
9. `test(data-health)`：集成/UI/本地化回归、长文案与 RTL 检查、构建验证和路线图状态回写。

每个提交只承担一个可验证目的；不在 P0-1 中混入会话搜索、预算、通知或清除 UI。

...
3. 新增 `TodayHourlyTokenLineChartView`。优点是可保持 popover 轻量,绘制逻辑可单独测试,并且只依赖已有 snapshot;缺点是需要新增一个小型自绘视图。
...
- 折线使用系统强调色。
- 数据点数量固定 24。
- 全零数据时不画折线峰值,只保持空网格和稳定 hover 行为。
- 当前小时可用较小圆点强调。
- 横轴只显示少量刻度,如 `0`、`6`、`12`、`18`、`23`,避免 24 个标签拥挤。
...
- 绘制和 hover 都基于 `MonthlyTokenBucket`。
...
- SwiftUI Charts 额外重量:本次使用 AppKit 自绘小视图,不新增桥接层。

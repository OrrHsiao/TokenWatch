...
3. 新增受版本控制的 `Info.plist`，在其中显式写入 `CFBundleLocalizations` 数组，并供 Debug 与 Release 共用。
...
Xcode 的生成式 `Info.plist` 只支持一组预定义的 `INFOPLIST_KEY_*` 构建设置，不能可靠写入数组类型的 `CFBundleLocalizations`。因此应用目标改为使用受版本控制的 `TokenWatch/Info.plist`：Debug 与 Release 都设置 `INFOPLIST_FILE = TokenWatch/Info.plist` 并关闭 `GENERATE_INFOPLIST_FILE`。该文件保留原有应用标识、版本、图标和 AppKit 主类所需的构建设置占位符，并显式声明 `CFBundleLocalizations`。
...
| 文件 | 改动 |
|---|---|
| `TokenWatch/Info.plist` | 新增带有 `CFBundleLocalizations` 的应用 Info 配置 |
| `TokenWatch/InfoPlist.xcstrings` | 新增 Bundle 层本地化目录 |
| `TokenWatch.xcodeproj/project.pbxproj` | 扩展区域声明并切换到受版本控制的 Info 配置 |

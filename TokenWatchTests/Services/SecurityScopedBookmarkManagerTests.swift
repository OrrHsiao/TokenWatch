...
struct SecurityScopedBookmarkManagerTests {
    ...
    func openPanelCopyUsesProviderMessage() {
        #expect(SecurityScopedBookmarkManager.openPanelCopy(
            for: ClaudeProvider(),
            language: .en
        ).message == "Choose the Claude Code data folder; it is usually ~/.claude.\nRun echo \"${CLAUDE_CONFIG_DIR:-$HOME/.claude}\" to find it.")
        #expect(SecurityScopedBookmarkManager.openPanelCopy(
            for: CodexProvider(),
            language: .en
        ).message == "Choose the Codex data folder; it is usually ~/.codex.\nRun echo \"${CODEX_HOME:-$HOME/.codex}\" to find it.")
        #expect(SecurityScopedBookmarkManager.openPanelCopy(
            for: OpenCodeProvider(),
            language: .en
        ).message == "Choose the opencode data folder; it is usually ~/.local/share/opencode.\nRun echo \"${XDG_DATA_HOME:-$HOME/.local/share}/opencode\" to find it.")
        #expect(SecurityScopedBookmarkManager.openPanelCopy(
            for: ClaudeProvider(),
            language: .en
        ).prompt == "Choose")
    ...
    func panelConfigurationUsesProviderCopyAndPreservesSystemDirectory() {
        ...
        #expect(panel.message == "Choose the Codex data folder; it is usually ~/.codex.\nRun echo \"${CODEX_HOME:-$HOME/.codex}\" to find it.")

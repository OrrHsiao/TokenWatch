...
| # | 项 | 影响 | 何时处理 |
|---|---|---|---|
| 1 | **200k tier 阶梯定价缺失**：ccusage `cost.rs::tiered_cost` 对超过 200k token 的部分使用 `*_above_200k` 单价；TokenWatch 全程一刀切。 | Claude Opus / Sonnet 大上下文请求**低估成本**（超出 200k 的部分实际更贵）。 | 用户开始关心账单准确性、或与 ccusage 对账出现系统性偏差时。 |
